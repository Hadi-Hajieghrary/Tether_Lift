The GPAC framework is a four-layer hierarchical controller that assigns each physical degree of freedom to a control layer operating at a progressively higher bandwidth (Fig.~\ref{fig:architecture}). Each agent~$i$ receives only: (i)~a shared reference trajectory~$p_d(t)$ (broadcast before flight), (ii)~its own IMU/GPS/barometer measurements, and (iii)~local cable tension $T_i$ and direction $q_i$. For the collision avoidance barrier, neighbor positions are obtained via GPS broadcast at 10\,Hz. No payload state, no other agent's cable state, and no centralized coordinator are required.

\begin{remark}[Per-agent sensing and communication]\label{rem:assumptions}
Each drone measures: IMU (200\,Hz), GPS (10\,Hz), barometer (25\,Hz), cable tension $T_i$ via load cell ($\sigma_T = 0.5$\,N), cable direction $q_i$ via 2-axis encoder ($\sigma_q = 0.02$\,rad). Communication is limited to: (d)~a shared precomputed trajectory $p_d(t)$, and (e)~neighbor GPS positions (10\,Hz) for collision avoidance only. No payload-mounted sensors, no centralized estimator, and no cable state or adaptive parameter exchange are needed.
\end{remark}
...
The outermost layer (${\sim}50$\,Hz) computes a desired thrust vector:
\begin{equation}
  F_{\text{des},i} = F_{\text{fb},i} + F_{\text{ff},i} + F_{\text{cable},i} + F_{\text{swing},i} + F_{\text{eso},i},
  \label{eq:Fdes}
\end{equation}
combining five physically motivated components. The PID trajectory tracking feedback (with per-axis anti-windup at $|e_{I,k}| \leq 2.0$) is
\begin{equation}
  F_{\text{fb},i} = -K_p\,e_{p_i} - K_d\,e_{v_i} - K_i\,e_{I_i},
  \label{eq:fb}
\end{equation}
with $K_p\!=\!\diag(6,6,8)$, $K_d\!=\!\diag(8,8,10)$, $K_i\!=\!\diag(0.1,0.1,0.2)$. The feedforward term compensates gravity and the nominal trajectory: $F_{\text{ff},i} = m_Q(a_{d_i} + ge_3)$.

Cable tension compensation prevents the position controller from treating cable force as a disturbance:
\begin{equation}
  F_{\text{cable},i} = \kappa_T(t)\,T_i\,q_i, \quad \kappa_T(t) = \min\!\bigl(1,\,(t - t_{\text{taut},i})/\Delta_{\text{ramp}}\bigr),
  \label{eq:cable_comp}
\end{equation}
where $t_{\text{taut},i}$ is the time cable~$i$ first reaches $T_i \geq 1.0$\,N and $\Delta_{\text{ramp}} = 2$\,s. The gain ramp gradually enables tension compensation using only the \emph{measured} cable force---no knowledge of $m_L$ or $N$ is required. This prevents impulsive loading during the slack-to-taut transition while remaining fully consistent with the decentralized estimation pipeline.

Anti-swing control suppresses pendular oscillations intrinsically on $\Sph^2$~\cite{lee2018geometric}:
\begin{equation}
  F_{\text{swing},i} = k_q\,e_{q_i} - k_\omega\,\omega_{q_i},
  \label{eq:swing}
\end{equation}
with $k_q\!=\!4.0$, $k_\omega\!=\!2.0$. Here $e_{q_i}$~\eqref{eq:eq} steers $q_i \to q_{d_i}$ along the geodesic and $\omega_{q_i} \in T_{q_i}\Sph^2$ is the cable angular velocity. Together, these terms reduce $\Psi_{q_i}$ monotonically absent external perturbation. The ESO disturbance estimate (Layer~4) is fed forward as $F_{\text{eso},i} = m_Q\hat{d}_i$.

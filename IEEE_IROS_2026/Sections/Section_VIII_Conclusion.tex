...
\begin{enumerate}
  ...
  \item The decentralized architecture achieves 23.8\,cm payload tracking RMSE in a high-fidelity Drake-based simulation with flexible bead-chain cables, onboard sensor fusion (ESKF), and Dryden wind turbulence, while requiring less than 1\,MFLOP/s per agent.
\end{enumerate}
...
The decentralized load estimator achieves 38.7\,cm RMSE compared to 12.4\,cm for the centralized baseline---a $3.1\times$ performance gap attributable to the single-cable geometric observability limitation~\eqref{eq:decentral_vs_central}. Distributed consensus-based estimation that fuses partial information from neighboring agents could narrow this gap while preserving communication efficiency.

\paragraph{CBF Conservatism}
The sequential gradient projection enforces constraints in priority order, which can be suboptimal when multiple constraints are simultaneously active. The cable angle constraint is violated for 3.2\% of the simulation time (within the ISSf margin), suggesting that a full QP-based multi-constraint resolution with slack variables could improve constraint satisfaction during aggressive maneuvers.

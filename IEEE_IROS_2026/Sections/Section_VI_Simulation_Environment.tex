...
\paragraph{Quadcopters} $N = 3$ free-floating rigid bodies, each with mass $m_Q = 1.5$\,kg and box inertia ($0.30 \times 0.30 \times 0.10$\,m). The quadrotor visual mesh is loaded from Drake's built-in URDF model and welded to each body for rendering. The quadcopters are initialized in a circular formation with radius $r_f = 0.5$\,m at altitude $z_0 = 1.2$\,m, centered above the payload:
\begin{equation}

...
As a comparison baseline for ablation studies (Section~\ref{sec:results:ablation}), we also evaluate a centralized EKF that fuses all available measurements. Although this estimator requires a central processor with access to all quadrotor states, it provides a performance upper bound for evaluating the decentralized approach. It is \emph{not} part of the online estimation pipeline; each agent runs only the decentralized estimator described above.

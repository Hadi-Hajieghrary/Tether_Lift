...
\begin{figure}[t!]
  ...
  \caption{CBF activation. \textit{Top:} Tension margin (distance to $T_{\min}$) for all cables. \textit{Middle:} Cable angle margin (distance to $\theta_{\max} = 34.4^\circ$); shaded regions indicate CBF activation during aggressive cornering (1.7\% of time). \textit{Bottom:} Tilt margin (never violated). Orange regions denote constraint activation.}
...
\begin{figure}[t!]
  ...
  \caption{Safety constraint satisfaction. \textit{Top:} Cable angle from vertical for each cable, with the CBF limit $\theta_{\max} = 34.4^\circ$. Brief excursions above the limit during aggressive cornering remain within the ISSf margin. \textit{Bottom:} Quadrotor tilt angle with limit $\phi_{\max} = 28.6^\circ$; the constraint is never violated.}
...
\end{align}
with $T_{\min}\!=\!2$\,N, $T_{\max}\!=\!60$\,N, $\theta_{\max}\!=\!0.6$\,rad ($34.4^\circ$), $\omega_{\max}\!=\!1.5$\,rad/s, $\phi_{\max}\!=\!0.5$\,rad ($28.6^\circ$), $d_{\min}\!=\!0.8$\,m. The first two barriers maintain cable tension within $[T_{\min}, T_{\max}]$; $h_\theta$ and $h_\omega$ limit cable angle and swing rate; $h_{\text{tilt}}$ bounds vehicle roll/pitch; and $h_{\text{col}}$ prevents inter-agent collision.
...
Compatibility relies on three well-separated timescales: \emph{fast} attitude ($k_R/J \approx 200$\,rad/s, settling $\sim$5\,ms), \emph{medium} safety filter (Butterworth cutoff $2\pi \times 15 \approx 94$\,rad/s), and \emph{slow} position/cable dynamics ($\sqrt{g/L} \approx 3$\,rad/s). Per~\cite[Ch.~11]{khalil2002nonlinear}, singular perturbation requires $\omega_{\text{att}}/\omega_{\text{CBF}} > 1/\epsilon$ for small $\epsilon$; here $200/94 \approx 2.1$ satisfies this with $\epsilon \approx 0.47$, and the attitude controller resolves any step change in $R_d^{\text{safe}}$ within one CBF cycle ($5$\,ms $\ll$ $11$\,ms filter delay). The Butterworth filter also rate-limits CBF force modifications, ensuring $R_d^{\text{safe}}$ varies slowly enough to maintain the $\Omega_{d_i} \approx 0$ simplification in~\eqref{eq:att_ctrl}. This bandwidth hierarchy $\omega_{\text{att}} \gg \omega_{\text{CBF}} \gg \omega_{\text{pos}}$ makes the layered architecture composable.
\end{remark}

...
The tautness barriers~\eqref{eq:h_taut} are relative-degree-one. Cable angle, tilt, and collision are relative-degree-two and use HOCBFs~\cite{ames2019control}: defining $\psi_1 = \dot{h} + \alpha_1 h$, the constraint $\dot{\psi}_1 + \alpha_2\psi_1 \geq 0$ becomes relative-degree-one in the control, with the safe set $\{\psi_1 \geq 0\} \cap \{h \geq 0\}$ rendered forward-invariant.
...
The implementation uses sequential gradient projection with priority ordering (tautness $>$ angle $>$ tilt $>$ swing $>$ collision), which is equivalent to the QP~\eqref{eq:cbf_qp} when at most one constraint is active---the typical case, as the CBF is active only 1.7--3.2\% of total simulation time. When multiple constraints activate simultaneously (rare; concentrated in $<$0.3\% of time during the sharpest cornering), the sequential method enforces the explicit priority ordering above, producing a feasible but potentially suboptimal solution; the slack variables $\delta_j$ ensure that lower-priority constraints degrade gracefully. The per-agent cost is $O(N_c)$ with $N_c = 6$ constraints (${\sim}1200$\,FLOPs/cycle).

...
Compatibility relies on three timescales: \emph{fast} attitude ($k_R/J \approx 200$\,rad/s, settling $\sim$5\,ms), \emph{medium} safety filter (Butterworth cutoff $2\pi \times 15 \approx 94$\,rad/s), and \emph{slow} position/cable dynamics ($\sqrt{g/L} \approx 3$\,rad/s). The attitude-to-CBF bandwidth ratio $200/94 \approx 2.1$ is moderate---classical singular perturbation~\cite[Ch.~11]{khalil2002nonlinear} typically requires this ratio to be $\gg\!1$---so we rely instead on a \emph{practical composability} argument: the attitude settling time ($\sim$5\,ms) is less than CBF Butterworth delay ($\sim$11\,ms), guaranteeing the attitude controller resolves any step change in $R_d^{\text{safe}}$ within one safety filter cycle. Combined with the Butterworth rate-limiting, which ensures $\norm{\dot{R}_d^{\text{safe}}} \leq 2\pi f_c \cdot 2\phi_{\max}$, the $\Omega_{d_i} \approx 0$ simplification in~\eqref{eq:att_ctrl} remains valid. The stronger separation $\omega_{\text{att}}/\omega_{\text{pos}} \approx 200/3 \approx 67$ does satisfy formal singular perturbation, ensuring the position-attitude cascade is stable; the CBF operates within this well-separated hierarchy.
\end{remark}

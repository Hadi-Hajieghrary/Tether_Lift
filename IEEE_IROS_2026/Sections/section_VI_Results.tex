...
Table~\ref{tab:failure_modes} consolidates hazard-to-mitigation mapping with constraint enforcement. The cable angle CBF is the most frequently active (1.7\% of time), with excursions to $48.1^\circ$ consistent with the ISSf bound~\eqref{eq:issf_bound}: for $\alpha_\theta = 2.0$ and measured $\bar{d} \approx 5$\,m/s$^2$, the predicted margin $(\mu_{\text{base}} + \kappa_d\bar{d})/\alpha_\theta \approx 4.75$\,m/s$^2$ corresponds to ${\sim}15^\circ$ allowance. The tilt constraint is never violated ($25.1^\circ$ max vs.\ $28.6^\circ$ limit). The CBF is active only 1.7--3.2\% of the time and increases tracking RMSE by less than 2\%.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llccc@{}}
    \toprule
    \textbf{Hazard} & \textbf{Mitigation} & \textbf{Limit} & \textbf{Observed} & \textbf{Active} \\
    \midrule
    Mass unknown & CL estimation & --- & $\hat{\theta} \!\to\! m_L/N$, 8\,s & --- \\
    Wind disturb. & ESO feedfwd & --- & +43\% w/o & --- \\
    Cable slack/overload & CBF tension & {[2, 60]\,N} & {[1.5, 26.3]\,N} & 0.2\% \\
    Cable angle & CBF + anti-swing & $34.4^\circ$ & $48.1^\circ$ & 1.7\% \\
    Excessive tilt & CBF tilt & $28.6^\circ$ & $25.1^\circ$ & None \\
    Swing rate & CBF swing & 1.5\,rad/s & 1.6\,rad/s & Brief \\
    Collision & CBF separation & 0.8\,m & $\geq 1.04$\,m & None \\

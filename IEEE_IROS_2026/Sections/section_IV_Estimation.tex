...
\begin{equation}
  ...
  \label{eq:regressor}
\end{equation}
where $\phi_i = \arccos(-n_{i,z})$ is the cable angle from vertical, $\hat{a}_L$ is the load acceleration (estimated via numerical differentiation with a first-order low-pass filter, $\tau_f = 0.1$\,s), and $\varepsilon_i$ captures modeling error from non-equilibrium dynamics and cable sag. The approximation is exact for vertical cables under quasi-static conditions; during aggressive maneuvering, the error is absorbed into the UUB convergence bound.

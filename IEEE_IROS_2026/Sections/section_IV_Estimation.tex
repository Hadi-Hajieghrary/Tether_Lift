...
\begin{remark}[Adaptations from~\cite{chowdhary2010concurrent}]
The scalar regressor~\eqref{eq:regressor} uses only local cable measurements (no full system model), the tension-confidence factor~\eqref{eq:tconf} prevents corruption during slack-to-taut transitions, and per-agent convergence~\eqref{eq:cl_conv} yields correct collective force~\eqref{eq:force_convergence} without consensus---while also isolating estimation faults to individual agents.
\end{remark}

...
The modeling error decomposes as $\varepsilon_i = \varepsilon_i^{\text{asym}} + \varepsilon_i^{\text{dyn}} + \varepsilon_i^{\text{sag}}$. (i)~\emph{Cable asymmetry:} With lengths $L_i = \bar{L}(1+\delta_i)$, force equilibrium under symmetric thrust yields $T_i\cos\phi_i = (m_Lg/N)(1 + \delta_i\sin^2\bar{\phi}\,/\cos\bar{\phi}) + O(\delta_i^2)$, so $|\varepsilon_i^{\text{asym}}| \leq (m_Lg/N)\,|\delta_i|\tan\bar{\phi}$. For $\delta_{\max}=0.19$ and $\bar{\phi} \leq \zeta_{\max}=34.4^\circ$, this gives $|\varepsilon_i^{\text{asym}}| \leq 1.28$\,N, or $0.13$\,kg equivalent. (ii)~\emph{Non-equilibrium dynamics:} Acceleration estimation error through the low-pass filter ($\tau_f = 0.1$\,s) contributes $|\varepsilon_i^{\text{dyn}}| \leq (m_L/N)\norm{\tilde{a}_L}/Y_i$, bounded by $O(\tau_f\norm{\dddot{p}_L})$; under minimum-jerk trajectories ($\norm{a_d}\leq 2$\,m/s$^2$), $|\varepsilon_i^{\text{dyn}}| \leq 0.4$\,N. (iii)~\emph{Cable sag:} Catenary departure introduces $|\varepsilon_i^{\text{sag}}| \leq m_{\text{rope}}g/(2N) = 0.33$\,N. The composite bound $\bar{\varepsilon} \leq 2.0$\,N ($\approx 0.2$\,kg equivalent) combined with the UUB result gives $|\tilde{\theta}| \leq \bar{\varepsilon}/(\rho\,\Sigma_Y) \approx 0.2/(0.5 \times 9.8) \approx 0.04$\,kg under nominal conditions—consistent with the $<0.1$\,kg observed in simulation with 19\% asymmetry.
\end{remark}
...
Proposition~\ref{prop:cl} analyzes parameter convergence with the regressor treated as an exogenous signal. In closed loop, the mass estimate $\hat{\theta}_i$ enters the feedforward $F_{\text{ff},i} = \hat{\theta}_i u$, which affects tracking error $e_{L,i}$, which perturbs cable measurements feeding back into the estimator. We argue this loop is stable via a small-gain composition. The feedforward-to-tracking transfer has gain $\norm{\tilde{\theta}_i u}/K_p \leq \tilde{\theta}_i\norm{u}/k_{p,\min}$; for $\norm{u}\leq g + a_{\max} \approx 11.8$\,m/s$^2$ and $k_{p,\min} = 6$, a mass error of $\tilde{\theta} = 0.1$\,kg produces tracking perturbation $\leq 0.2$\,m. This perturbation enters the cable measurement as a geometric offset $\leq 0.2/L \approx 0.2$\,rad in cable angle, contributing $\leq 0.2 T_i \approx 2.4$\,N to the regressor residual---within the $\bar{\varepsilon}$ bound of Remark~\ref{rem:eps_bound}. Thus the closed-loop coupling is absorbed into the existing UUB analysis without changing the convergence guarantee. The cascade estimation architecture (ESKF at 200\,Hz $\to$ load filter at 50\,Hz $\to$ mass estimator at 50\,Hz) further attenuates the coupling by enforcing unidirectional error propagation with $4\times$ bandwidth separation at each stage.
\end{remark}
